...
\icmltitlerunning{RE-Clust: Restaurant Based Clustering}

\begin{document}
\twocolumn[
   \icmltitle{RE-Clust: Restaurant Based Clustering}
...
\begin{abstract}
   ...
   We say that a ``competitor'' is any similar business, with the idea that a similar business caters to a similar clientele and is therefore a competitor.
   We utilize a clustering method with customized distance metrics to find similar businesses.

   We then evaluate our methods on two data sets: one created computationally and one manually labeled by humans.
   Evaluation on both sets yields a high Rand index.
\end{abstract}
